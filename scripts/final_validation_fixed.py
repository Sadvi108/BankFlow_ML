...
def final_validation_test_fixed():
    """Final comprehensive validation test with proper matching"""
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        # Check if expected ID was found (case-insensitive comparison)
        found_expected = any(
            extracted_id['text'].upper().replace("-", "") == expected.upper().replace("-", "") 
        ...
        logger.info(f"{status} Test {i}: Expected {expected}, Found {extracted_ids}")
...
if __name__ == "__main__":
    success_rate = final_validation_test_fixed()
