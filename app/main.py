...
if supabase:
    ...
    print("Using local JSONL storage")
    storage = dataset
...
async def extract_fields(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Fallback to local storage if using Supabase
        if storage != dataset:
            print("Supabase write failed. Falling back to local storage.")
            try:
                dataset.append_annotation(entry)
                # Continue success flow, but maybe log it
            except Exception as e2:
                return JSONResponse(
                    status_code=500,
                    content={"success": False, "detail": f"Failed to save to Supabase ({e}) AND local storage ({e2})"}
                )
        else:
            return JSONResponse(
                status_code=500,
                content={"success": False, "detail": f"Failed to save to database: {str(e)}"}
            )
