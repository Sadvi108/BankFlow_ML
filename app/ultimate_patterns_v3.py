...
class UltimatePatternMatcherV3:
    ...
    def extract_amount(self, text: str) -> Optional[str]:
        ...
        header_patterns = [
            ...
            r'\bAmount\s*[:\s]*(?:RM|MYR)\s*([\d,]+\.\d{2})',
            r'\bTotal\s*[:\s]*(?:RM|MYR)\s*([\d,]+\.\d{2})',
            # "Transfer Amount (MYR) : 154.00" / "Total Amount (RM) : 20.00"
            r'\b(?:Transfer|Transaction|Total)?\s*Amount\s*\(?\s*(?:RM|MYR)?\s*\)?\s*[:\s]*([\d,]+\.\d{2})',
            # "Payment Amount MYR 100.00"
            r'\b(?:RM|MYR)\s*[:\)]?\s*([\d,]+\.\d{2})',
            # "Amount (MYR)\n2,710.00" — label then number on next line
            r'\bAmount\b[^\n]{0,20}\n\s*(?:RM|MYR)?\s*([\d,]+\.\d{2})',
        ]
        # Integer amounts (no decimals): "RM 150", "Amount: RM 70"
        int_patterns = [
...
_DATE_RES = [
    ...
    re.compile(r'^\d{1,2}[-/ ](\d{1,2}|[A-Za-z]{3,9})[-/ ]\d{2,4}' + _TIME_TAIL),
    # ISO YYYY-MM-DD
    re.compile(r'^\d{4}[-/]\d{1,2}[-/]\d{1,2}' + _TIME_TAIL),
    # Month-name D, YYYY  (Jan 11, 2021 / January 19, 2021)
    re.compile(r'^[A-Za-z]{3,9}\.? \d{1,2},? \d{4}' + _TIME_TAIL),
]
_ID_RE = re.compile(r'^[A-Z0-9][A-Z0-9\-/]{3,29}$', re.IGNORECASE)


def validate_id(value) -> bool:
    if value is None:
        return False
    s = str(value).strip().upper().replace(' ', '')
    if not s or not _ID_RE.match(s):
        return False
    for pat in _SST_PATTERNS:
        if pat.match(s):
            return False
    # Must contain at least one digit (rejects pure-letter words like "REFERENCE")
...
def validate_amount(value) -> bool:
    if value is None:
        return False
    s = str(value).strip().replace('RM', '').replace('MYR', '').strip()
...
def validate_date(value) -> bool:
    if value is None:
        return False
    s = str(value).strip()
    return any(rx.match(s) for rx in _DATE_RES)
